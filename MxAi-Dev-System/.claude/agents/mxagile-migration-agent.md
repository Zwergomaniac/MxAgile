...
14. **Melde Migration als abgeschlossen** -- erst jetzt, nach Schritt 13.
    Das Projekt befindet sich jetzt im **Hybrid-Modus**:
    - `artifact_canonicalization: pending` ist in `state.yaml` gesetzt
      (gesetzt durch install-core.ps1 Step 1d bei der Installation)
    - Legacy-Artefakte in `planning/stories/` und `planning/checklists/` sind erhalten und lesbar
    - Normale MxAgile-Projektarbeit (Discovery, Refinement, Implementing, ...) kann jetzt beginnen
    - Artefakt-Kanonisierung ist ein **separater, optionaler naechster Lifecycle-Schritt**:
      Skill `.mxagile/skills/migration.md` (Artifact Canonicalization)
