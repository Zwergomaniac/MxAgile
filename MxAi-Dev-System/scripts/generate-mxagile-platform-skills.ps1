...
.DESCRIPTION
    ...
    directories (.claude/, .github/, .agents/, .grok/, .opencode/, .hermes/).
    ...
    Directory naming for non-Claude/Copilot platforms uses "mxagile/" (not "dfc/").
...
$mxagileRoot = Join-Path $ProjectRoot '.mxagile'
...
if (-not (Test-Path $mxagileRoot)) {
    Write-Error ".mxagile/ directory not found at $mxagileRoot"
    exit 1
}
...
function Generate-ClaudeSkills {
    $skillsDir = Join-Path $mxagileRoot 'skills'
...
function Generate-ClaudeAgents {
    $agentsDir = Join-Path $mxagileRoot 'agents'
...
function Generate-CodexSkills {
    $skillsDir = Join-Path $mxagileRoot 'skills'
    $outDir = Join-Path $ProjectRoot ".agents/skills/mxagile"
    foreach ($f in (Get-ChildItem $skillsDir -Filter '*.md')) {
        $name = $f.BaseName
        $body = Get-Content $f.FullName -Raw -Encoding UTF8
...
function Generate-GrokSkills {
    $skillsDir = Join-Path $mxagileRoot 'skills'
    $outDir = Join-Path $ProjectRoot ".grok/skills/mxagile"
    foreach ($f in (Get-ChildItem $skillsDir -Filter '*.md')) {
        $name = $f.BaseName
        $body = Get-Content $f.FullName -Raw -Encoding UTF8
        $outPath = Join-Path $outDir "$name.md"
...
function Generate-OpenCodeSkills {
    $skillsDir = Join-Path $mxagileRoot 'skills'
    $outDir = Join-Path $ProjectRoot ".opencode/skills/mxagile"
    foreach ($f in (Get-ChildItem $skillsDir -Filter '*.md')) {
        $name = $f.BaseName
        $body = Get-Content $f.FullName -Raw -Encoding UTF8
        $outPath = Join-Path $outDir "$name.md"
...
function Generate-HermesSkills {
    $skillsDir = Join-Path $mxagileRoot 'skills'
    $outDir = Join-Path $ProjectRoot ".hermes/skills/mxagile"
    foreach ($f in (Get-ChildItem $skillsDir -Filter '*.md')) {
        $name = $f.BaseName
        $body = Get-Content $f.FullName -Raw -Encoding UTF8
        $outPath = Join-Path $outDir "$name.md"
