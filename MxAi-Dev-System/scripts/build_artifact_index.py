...
def build_graph(artifacts):
    ...
    # First pass: create all nodes and gather all known IDs
    ...
    # Second pass: create edges by finding relationships
    for node in nodes:
        node_id = node['id']
        content = node.get('content', {})

        # Relationship: Requirement derived from Page (Page -> Requirement)
        ...
        # Relationship: Spec implements Requirement (Requirement -> Spec)
        if node['type'] == 'spec':
        ...
        # Relationship: Task implements Spec (Spec -> Task)
        if node['type'] == 'task':
...
def main():
    # Correctly handle argument parsing for both direct and --path invocations
    ...
    # Stage 1: Find all artifacts
    artifacts = find_artifacts(project_root)

    # Stage 2: Build graph from artifacts
    graph = build_graph(artifacts)

    # Stage 3: Save the final graph
