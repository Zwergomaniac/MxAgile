...
- **Commits vorbereiten, dann bestaetigen lassen** — Agenten duerfen Staging und Commit-Vorschlag vorbereiten und `git add` sowie `git commit` nach ausdruecklicher Bestaetigung im selben Turn oder im explizit aktivierten Autopilot-Modus ausfuehren. `git push` sowie destruktive History-Aenderungen bleiben manuell.
...
**MPR-v2-Sperre: `mxcli docker check` NICHT auf dem kanonischen Checkout ausfuehren.**
Dieser Befehl ist in MPR-v2-Projekten keine read-only-Pruefung: Er kann mit
`Updating widget definitions` den Checkout materialisieren oder teilweise leeren.
Vor jedem notwendigen Check eine Recovery-Kopie unter `.concord/scratch/` erstellen;
fuer reine MDL-/Referenzvalidierung `mxcli check <script>.mdl -p <project>.mpr --references`
verwenden.
